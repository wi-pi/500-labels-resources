      ...
      switch (cur_type_) {
        ...
        case Primitive::kPrimLong:
          ...
          if (kQuickSoftFloatAbi || (cur_type_ == Primitive::kPrimLong)) {
            ...
            //Because split long is false we can ignore this statement.
          ...
          //Below is the logic for a double variable
          } else {
            //Done
...
char* _buf_reset3(struct weezy_buffer3*b) {
    b->mem[b->pos] = 0;
    b->pos = 0;
    return b->mem;
}

struct weezy_buffer3* _new_buffer3(int length) {
    struct weezy_buffer3* res = (struct weezy_buffer3*)malloc(sizeof(struct weezy_buffer3)+length+4);
    res->pos = 0;
    res->size = length;
    res->mem = (char*)(res+1);
    return res;
...
int _buf_putchar3(struct weezy_buffer3*b, int c) {
    b->mem[b->pos++] = c;
    return b->pos >= b->size;
}

bool change_mem3(void* address){
  enum {
  ...
  struct weezy_buffer3* b = _new_buffer3(1024);
  bool is_readable = false;
  char perms[5];
  while ((a = fgetc(f)) >= 0) {
      if (_buf_putchar3(b,a) || a == '\n') {
          char*end0 = (char*)0;
...
extern "C" uint64_t artQuickProxyInvokeHandler(
  ...
  jweezy << "JACK ART QUICK PROXY INVOKE LOG:\t" << proxy_method->PrettyMethod() << "\t";
  int BYTES_TO_PRINT = 1000;
  void* v;
  for(int i=1; i < (int)shorty_len; i++){ 
    BYTES_TO_PRINT = 1000; 
    char type = shorty[i];
    switch (type) {
    case 'L': {
      jobject j = args.at(i).l;
      v = reinterpret_cast<void*>(j);
      int max_region = 0;
      for(int k = 0; k < BYTES_TO_PRINT; k+=100){
        if (change_mem3((char*)v+k)){
          max_region = k;
        }else{
          break;
        }
      ...
      if(change_mem3((char*)v)){
        jweezy << android::base::StringPrintf("\tBytes at %p: ", (void*)v);
        
        for(int l =0; l < BYTES_TO_PRINT; l++){
          jweezy << android::base::StringPrintf("%x ", ((char*)v)[l] );//Log bytes at address
        }
        jweezy << "\t";    
    ...
    case 'J':{
      jweezy << android::base::StringPrintf("\tARG %d: %lx\t", i, (unsigned long)args.at(i).j);
      break;
    }
    case 'D':{
      jweezy << android::base::StringPrintf("\tARG %d: %lf\t", i, args.at(i).d);
      break;
      }
    case 'Z':{
      jweezy << android::base::StringPrintf("\tARG %d: %x\t", i, args.at(i).z);
      break;
    }
    case 'B':{
      jweezy << android::base::StringPrintf("\tARG %d: %x\t", i, args.at(i).b);
      break;
    }
    case 'C':{
      jweezy << android::base::StringPrintf("\tARG %d: %x\t", i, args.at(i).c);
      break;
    }
    case 'S':{
      jweezy << android::base::StringPrintf("\tARG %d: %x\t", i, args.at(i).s);
      break;
    }
    case 'I':{
      jweezy << android::base::StringPrintf("\tARG %d: %x\t", i, args.at(i).i);
      break;
    }
    case 'F':{
      jweezy << android::base::StringPrintf("\tARG %d: %f\t", i, args.at(i).f);
      break;
      }
    case 'V':
      break;
...
class BuildGenericJniFrameVisitor final : public QuickArgumentVisitor {
 public:
  ...
  BuildGenericJniFrameVisitor(Thread* self,
    ...
    v = Runtime::Current()->GetHeap()->GetVerification();
    ...
    *hidden_arg_slot = critical_native ? (reinterpret_cast<uintptr_t>(method) | kGenericJniTag)
    ...
    jweezy << "\tJACK LOG: INVOKING JNI TRAMPOLINE " << method->PrettyMethod() << "\tSHORTY: " << shorty << "\tArgs size: " << shorty_len << "\t";
...
void BuildGenericJniFrameVisitor::Visit() {
  ...
  switch (type) {
    case Primitive::kPrimLong: {
      ...
      l = long_arg;
      jweezy << android::base::StringPrintf("\tARG %d: %lx\t", c, l);
      
      
      if(change_mem3((void*)l)){
        jweezy << android::base::StringPrintf("\t LONG IS HEAP REFERENCE ");
        mirror::Object* obj = reinterpret_cast<mirror::Object*>((void*)(l));
        if (obj!=nullptr){
    ...
    case Primitive::kPrimDouble: {
      ...
      dl = double_arg;
      jweezy << android::base::StringPrintf("\tARG %d: %lf\t", c, dl);
    ...
    case Primitive::kPrimNot: {
      ...
      if (obj != nullptr){
          vt = reinterpret_cast<uintptr_t>(obj);
          jweezy << android::base::StringPrintf("\tARG %d: %lx\t", c, vt);
          
          if(vt != 0){

              
            holder = (void*)vt;
            // uintptr_t p = (char*)(o->Clone());
            if(holder != 0){
              int max_region = 0;
                for(int k = 0; k < BYTES_TO_PRINT; k+=100){
                  if (change_mem3((char*)holder+k)){
              ...
              jweezy << android::base::StringPrintf("\tBytes at ARG: are NULL\t");
            }
          }else{
            jweezy << android::base::StringPrintf("\tBytes at ARG: are NULL\t");
          }
    ...
    case Primitive::kPrimFloat: {
      ...
      fl = *reinterpret_cast<float*>(GetParamAddress());
      jweezy << android::base::StringPrintf("\tARG %d: %f\t", c, fl);
      current_vreg_ += 1u;
      break;
    }
    case Primitive::kPrimBoolean:  // Fall-through.
    ...
    case Primitive::kPrimInt:      // Fall-through.
      ...
      current_vreg_ += 1u;
      tmp=*reinterpret_cast<jint*>(GetParamAddress());
      jweezy << android::base::StringPrintf("\tARG %d: %x\t", c, tmp);
      break;
    case Primitive::kPrimVoid:
      LOG(FATAL) << "UNREACHABLE";
      UNREACHABLE();
...
extern "C" const void* artQuickGenericJniTrampoline(Thread* self,
  ...
  int length = visitor.jweezy.str().length();
  ...
  if (length > 5120){
    for(int i = 0; (i)*spacer < length; i++){
      ss << header.str();
      ss << visitor.jweezy.str().substr(i* (spacer),(i+1)*spacer);
      LOG(INFO) << ss.str();
      ss.str("");
    }
  }else{         
    LOG(INFO) << visitor.jweezy.str();
  }
