...
#include "android-base/stringprintf.h" //For StringPrintf
...
static JniCompiledMethod ArtJniCompileMethodInternal(const CompilerOptions& compiler_options,
  ...
  // 6. Fill arguments. IMPORTANT COME BACK HERE LATER
  if (UNLIKELY(is_critical_native)) {
  ...
  // int size = code.size();
  // uint8_t* start = code.begin();
  // jweezy << android::base::StringPrintf("JACK COMPILER LOG:\t");
  // jweezy << dex_file.PrettyMethod(method_idx) << "\t";
  // for(int i=0; i<size; i++){
  //   jweezy << android::base::StringPrintf("%.2x", (char)( *(start+i) ));
  // }
  ...
  // header << "JACK COMPILER LOG: CONT.\t" << dex_file.PrettyMethod(method_idx) << "\t";
  // if (length > 5120){
