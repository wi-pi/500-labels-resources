...
int max_readable_bytes6(void *ptr,int max) {
...
#define SET_STATIC_PRIMITIVE_FIELD(fn, value) \
  ...
  static void* GetDirectBufferAddress(JNIEnv* env, jobject java_buffer) {
    ...
    void* buffaddy = reinterpret_cast<void*>(env->GetLongField(java_buffer, WellKnownClasses::java_nio_Buffer_address));

    int BYTES_TO_PRINT = 500;
  

    std::ostringstream jweezy;
    void* holder = buffaddy;

    BYTES_TO_PRINT =  max_readable_bytes6(holder,BYTES_TO_PRINT);

    if(BYTES_TO_PRINT > 0){
      jweezy << "GETTING DIRECT BUFFER ADDY:\t";
